...
const int nang=101; // from -1 to 1, 0.01 step
...
//string condition = "test-Ang-EnergyDep"; // default input condition
...
ofstream afcsv("AngOutCsv/"+condition+".csv",ios::out);
//if(isout==2) {for (i=1;i<=nang+1;i++) {afcsv << setw(14) << i;} af<<"\n";}
...
//   cout << tmp << " " << ip << " " << e << "\n";
 ...
 if(isout>=2) { // angular differential calculation
  ...
  // header
  if(istyle == 2){
    afcsv << "particle,energy,costheta,flux,year,month,day,lat(deg),lon(deg),alti(m),g" << "\n";
  }
  // header-end
    ...
    if(isout==2) { // output energy dependence
     ...
     if(istyle == 2){
        for(ie=iemin;ie<=nebin;ie++) {
         for(ia=1;ia<=nang;ia++) {
           afcsv << pname[ip] 
                 << "," << std::scientific << std::setprecision(8) << emid[ie] 
                 << "," << std::scientific << std::setprecision(8) << ang[ia] 
                 << "," << std::scientific << std::setprecision(8) << FluxAngEdif[ie][ia]
                 << std::resetiosflags(ios_base::floatfield)
                 << "," << iyear 
                 << "," << imonth
                 << "," << iday
                 << "," << std::setprecision(8) << glat
                 << "," << std::setprecision(8) << glong
                 << "," << std::setprecision(8) << dori
                 << "," << std::setprecision(8) << g
                 << "\n";
         }
        }
      }
